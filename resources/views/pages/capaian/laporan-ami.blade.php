@extends('layouts.app')
@section('title', 'Laporan AMI - LPM Politeknik Jambi')
@section('content')
    ...
    <div class="relative z-10 border-b border-slate-200 dark:border-white/10 bg-gradient-to-r from-blue-50/50 dark:from-blue-900/40 via-slate-100/50 dark:via-slate-900/60 to-indigo-50/50 dark:to-indigo-900/40 py-16 transition-colors duration-500">
        <div class="max-w-7xl mx-auto px-6 lg:px-16">
            <div class="flex items-center gap-3 text-blue-600 dark:text-blue-400 text-xs font-bold uppercase tracking-widest mb-4">
                ...
                <i class="fas fa-chevron-right text-[8px] text-slate-400 dark:text-slate-600"></i>
                <span class="text-slate-950 dark:text-white">Laporan AMI</span>
            </div>
            <div class="flex flex-col md:flex-row items-start md:items-end gap-6 justify-between">
                <div>
                    ...
                    <p class="text-slate-600 dark:text-slate-400 text-base max-w-2xl leading-relaxed transition-colors duration-500">
                        Kumpulan Laporan Audit Mutu Internal (AMI) Politeknik Jambi.
                ...
                <div class="flex items-center gap-3 bg-white/60 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl px-5 py-3 backdrop-blur-md transition-colors duration-500">
                    ...
                    <div>
                        ...
                        <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Total Laporan</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid lg:grid-cols-4 gap-10">
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-white/80 dark:bg-slate-800/60 backdrop-blur-md rounded-3xl border border-slate-200 dark:border-white/10 p-6 sticky top-6 shadow-sm transition-colors duration-500">
                    ...
                    <form method="GET" action="{{ route('capaian.laporan_ami') }}" id="filterForm">
                        {{-- Filter Tahun --}}
                        <div class="mb-5">
                        ...
                        <a href="{{ route('capaian.laporan_ami') }}" 
                    ...
                    <div class="mt-6 pt-5 border-t border-slate-200 dark:border-white/10 transition-colors duration-500">
                        ...
                        <div class="space-y-2">
                            ...
                            @php $countTh = \App\Models\LaporanAmi::where('tahun', $th)->count(); @endphp
                            <div class="flex items-center justify-between text-sm">
                                <a href="{{ route('capaian.laporan_ami', ['tahun' => $th]) }}"
            ...
            <div class="lg:col-span-3">
                ...
                <div class="flex flex-wrap items-center gap-3 mb-6">
                    ...
                    @endif
                    <span class="text-slate-500 text-xs transition-colors">{{ $dokumen->count() }} laporan ditemukan</span>
                </div>
                @endif
                    ...
                    <div class="flex items-center gap-4 mb-5">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{{ $docs->count() }} laporan</div>
                            </div>
                        </div>
                            ...
                            <table class="w-full text-left border-collapse">
                                <thead>
                                    <tr class="bg-slate-50/50 dark:bg-white/5 border-b border-slate-200 dark:border-white/10 transition-colors duration-500">
                                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em] transition-colors">Laporan</th>
                                ...
                                <tbody class="divide-y divide-slate-100 dark:divide-white/5 transition-colors duration-500">
                                    ...
                                    <tr class="group hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors {{ $loop->iteration > 3 ? 'hidden extra-row-'.Str::slug($tahun) : '' }}">
                                        <td class="px-6 py-5">
                                            <div class="flex items-center gap-4">
                                                ...
                                                <div class="min-w-0">
                                                    ...
                                                    @if($dok->kategori && $dok->kategori !== 'Laporan AMI')
                                        ...
                                        <td class="px-6 py-5 text-right">
                                            @if($dok->path_file)
                                            <a href="{{ route('laporan_ami.download', $dok->id) }}"
                ...
                <div class="flex flex-col items-center justify-center py-24 text-center">
                    ...
                    <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-2 transition-colors">Belum Ada Laporan</h3>
                    <p class="text-slate-600 dark:text-slate-500 text-sm max-w-sm leading-relaxed transition-colors">
                        @if(request('tahun') || request('kategori'))
                            Tidak ada laporan yang sesuai dengan filter yang dipilih. Coba ubah filter atau
                            <a href="{{ route('capaian.laporan_ami') }}" class="text-blue-600 dark:text-blue-400 hover:underline">tampilkan semua laporan</a>.
                        @else
                            Laporan AMI akan segera tersedia. Silakan kunjungi kembali beberapa saat lagi.
                        @endif
                    </p>
                </div>
                @endif
